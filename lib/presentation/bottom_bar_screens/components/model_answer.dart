...
class ModelAnswerScreen extends StatelessWidget {
   ModelAnswerScreen({super.key , required this.quizId});
  String quizId;
  @override
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: BlocBuilder<GetModelAnswersBloc,GetModelAnswersState>(
        builder: (context,state) {
          if(state is GetModelAnswersState && state.requestState ==RequestState.loading ){
            return Skeletonizer(child: _TaskAnswerBody(answerCorrectEntity: [AnswerCorrectEntity(question: "1", studentAnswer: "1", correctAnswer: "1")],));
          }
          if(state is GetModelAnswersState && state.requestState ==RequestState.done ){
            return _TaskAnswerBody(answerCorrectEntity: state.getAnswerQuestionResponse,);
          }
          else {
            return GestureDetector(
              onTap: (){
                context.read<GetModelAnswersBloc>().add(GetModelAnswersRequestEvent(quizId: quizId,studentId:context.read<CurrentUserBloc>().userData!.id));
              },
              child: Container(
...
class _TaskAnswerBody extends StatelessWidget {
  ...
  List<AnswerCorrectEntity> answerCorrectEntity;
